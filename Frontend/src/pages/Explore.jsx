...
const Explore = () => {
  ...
  const handleSearch = () => {
    ...
    if (filter.location.trim()) {
      const searchLocation = filter.location.trim().toLowerCase();
    ...
    if (filter.idealFor) {
      result = result.filter((property) => {
        if (!Array.isArray(property.idealFor)) {
          return false;
        }

        return property.idealFor.includes(filter.idealFor);
      });
    }
    ...
    if (filter.amenities.length > 0) {
      result = result.filter((property) => {
        ...
        return filter.amenities.every((amenity) =>
    ...
    if (filter.bedroom) {
      result = result.filter((property) => {
        return Number(property.bedrooms) === Number(filter.bedroom);
      });
    }
    ...
    console.log("Search completed:", result.length);
  };
      ...
      <div className="relative">
        ...
        >
          ...
          >
            ...
            <motion.div
              variants={item}
              className="mt-10 max-w-2xl hidden md:flex"
            >
            ...
            <div className="flex flex-col sm:flex-row items-center sm:items-start gap-6 text-center sm:text-left py-10">
              ...
              <div>
                <h2 className="text-3xl md:text-4xl font-serif font-semibold text-white">
                  You've Saved the Best. Now Choose
                  One.
                </h2>

                <p className="mt-3 text-primary-100 text-base md:text-lg max-w-xl leading-7">
                  Explore your wishlist and schedule a
                  visit for the properties that caught
                  your eye.
                </p>
