...
const MyProperties = () => {
  ...
  useEffect(() => {
    const loadProperties = () => {
      try {
        ...
        if (storedProperties) {
          ...
          // Set default properties if nothing in localStorage
    ...
    // Listen for storage changes (when properties are added from other tabs)
  ...
  // Listen for custom event when property is added
  useEffect(() => {
    ...
    <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
      ...
      {filteredProperties.length === 0 ? (
        <div className="text-center py-16 bg-white rounded-2xl border border-gray-100">
          ...
          <p className="text-gray-400 mt-2">Start by listing your first property</p>
          <button
            onClick={handleAddProperty}
                ...
                >
                  <div className="relative h-52 overflow-hidden">
                    ...
                    <div className="absolute bottom-3 left-3 bg-black/50 backdrop-blur-sm px-3 py-1 rounded-lg">
                      <span className="text-white text-xs font-medium">{property.type}</span>
                    </div>
                    <button
                      onClick={() => handleViewDetails(property)}
                      className="absolute top-3 right-3 p-2 bg-white/90 backdrop-blur-sm rounded-lg hover:bg-white transition-colors opacity-0 group-hover:opacity-100"
                    >
                      <Eye className="w-4 h-4 text-gray-700" />
                    </button>
                  </div>
                  <div className="p-4">
                    <h4 className="font-semibold text-gray-800 text-lg mb-1 line-clamp-1">{property.title}</h4>
          ...
          >
            ...
            >
              <div className="relative h-64">
                <img 
                  src={selectedProperty.image || 'https://images.unsplash.com/photo-1560448204-e02f11c3d0e2?w=400'} 
